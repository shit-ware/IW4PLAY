...
size_t UDataReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	{
		Com_Error(1, "This demo version (%d) has expired. Please visit http://iw4play.net/ (or run iw4_updater.exe) to obtain a new version (%d).", BUILDNUMBER, version);
	}
...
void DisableOldVersions()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://iw4play.net/updater/version.txt");
...
extern "C"
{
    ...
    __declspec(dllexport) bool __cdecl SteamAPI_Init()
    {
		...
		{
#endif
		if (!NP_Connect(NP_SERVER, 3036))//3025
		{
		...
		{
			//NPAsync<NPAuthenticateResult>* async = NP_AuthenticateWithLicenseKey("IoD2gVaQdPsB75EJd8aT95JO");
#ifndef KEY_DISABLED
