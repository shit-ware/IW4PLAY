...
bool Auth_DisplayLoginDialogDownLevel(const wchar_t** usernameP, const wchar_t** passwordP, PCREDUI_INFOW info)
{
	...
	HRESULT result = CredUIPromptForCredentialsW(info, L"iw4play.net", NULL, NULL, username, CREDUI_MAX_USERNAME_LENGTH, password, CREDUI_MAX_PASSWORD_LENGTH, &save, CREDUI_FLAGS_GENERIC_CREDENTIALS | CREDUI_FLAGS_ALWAYS_SHOW_UI | CREDUI_FLAGS_DO_NOT_PERSIST | CREDUI_FLAGS_EXCLUDE_CERTIFICATES);
...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	info.pszMessageText = L"Please log in to use IW4M. Use your iw4play.net forum credentials.";
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://%s/remauth.php", "iw4play.net/authorisation");
		...
		{
			Auth_Error(va("Could not reach the iw4play.net server. Error code from CURL: %x.", code));
		}
...
void Auth_OpenDLCStore()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://iw4play.net/authorisation/iw4/dlc.txt");
