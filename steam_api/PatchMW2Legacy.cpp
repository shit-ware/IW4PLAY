...
void Legacy_Advertise()
{
	if ((Com_Milliseconds() - lastAd) > 120000)
	{
		for (int i = 0; i < *svs_numclients; i++)
		{
			...
			SV_GameSendServerCommand(i, 0, va("%c \"This server is running ^2IW4Play - Modifikation.\"", 104));
			SV_GameSendServerCommand(i, 0, va("%c \"Get the client at http://iw4play.net/\"", 104));
		}
